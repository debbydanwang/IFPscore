...
para = {'alg_type': 'qf_pc', # 'avg', 'classic', 'quasi' or 'qf_pc'
...
r1 = validate_model(mdl = model, vset = test_dataset_cs)
r2 = validate_model(mdl = model, vset = test_dataset_csar1)
r3 = validate_model(mdl = model, vset = test_dataset_csar2)
r4 = validate_model(mdl = model, vset = test_dataset_csar3)
  
# remove tmperary files
drs = ['/tmp/' + fd for fd in os.listdir('/tmp') if 'tmpdt' in fd]
for dr in drs:
    if os.path.isdir(dr):
